// config.example.js — TEMPLATE. Copy this to config.js and fill in your
// real Supabase project values. config.js itself must stay out of git
// (add it to .gitignore) since it holds your real project URL/key.

window.KOE_CONFIG = {
  SB_URL: 'YOUR_SUPABASE_URL',   // e.g. https://xxxxxxxxxxxx.supabase.co
  SB_KEY: 'YOUR_SUPABASE_PUBLISHABLE_KEY'  // starts with sb_publishable_
  S3_FN_URL: "https://tmqzqlrpjpydiftlrzmj.supabase.co/functions/v1/smart-function"
};
