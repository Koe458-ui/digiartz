/* ── effects.js · ripple, ads panel, legal modals, FAQ mount, section scope ── */
...
(function () {
  ...
  /* ── Sections in scope ── */
  var SCOPE_IDS = ['artworks', 'rankSec', 'subOverview', 'connect'];

  /* ── CSS selectors to animate (within scope sections) ── */
  var TARGETS = [
    /* Section headers */
    '.fsHdr', '.awHdr', '.subOvHdr',
    /* Individual heading / sub elements (fallback if parent already targeted) */
    '.secT', '.secSub',
    /* Artworks section cards */
    '.awCard',
    /* Subscription overview cards */
    '.subOvCard',
    /* Ranking boards — same rise-in as the subscription cards below them.
       (.rkHdr already carries .fsHdr, so the section heading is covered.) */
    '.rkCard',
    /* Buttons */
    '.upBtn',
    /* Empty states */
  ...
  /* ── Staggered groups: cards & grid items get cascade delays ── */
  var STAGGER_SELECTORS = ['.awCard', '.rkCard', '.subOvCard', '.sCard'];
  var STAGGER_BASE_MS   = 80;   /* ms between each sibling card */
  var STAGGER_MAX_MS    = 560;  /* cap so late cards don't wait forever */

  /* ── Stamp .fu-el on every target inside the scoped sections ── */
  function stampElements() {
    SCOPE_IDS.forEach(function (id) {
      var section = document.getElementById(id);
      if (!section) return;

      /* NOTE: the section wrapper itself is intentionally NOT
         stamped with .fu-el anymore. Animating the section root
         made its own background fade from opacity:0, which let
         the white body background show through as a gap/line
         above each section until it scrolled into view. Only
         inner content now animates; the dark section background
         stays visible at all times, keeping the theme continuous. */

      /* Animate all matching children */
      var children = section.querySelectorAll(TARGETS);
      children.forEach(function (el) {
        /* Skip elements that are already a scoped section root */
        if (SCOPE_IDS.indexOf(el.id) !== -1) return;
        addFuEl(el);
      });
    });
  }
  ...
  function addFuEl(el) {
    /* Never double-stamp */
    if (el.classList.contains('fu-el')) return;
    el.classList.add('fu-el');
  }

  /* ── Apply stagger delays to sibling card groups ── */
  function applyStagger() {
    SCOPE_IDS.forEach(function (id) {
      ...
      STAGGER_SELECTORS.forEach(function (sel) {
        var cards = section.querySelectorAll(sel);
        cards.forEach(function (card, i) {
          /* FIX: never re-stamp a delay onto an already-revealed card —
             the reveal handler just cleared it to unblock hover
             transitions; re-applying it here would bring the lag back
             on every dynamic restamp. */
          if (card.classList.contains('fu-visible')) return;
          var delay = Math.min(i * STAGGER_BASE_MS, STAGGER_MAX_MS);
  ...
  /* ── IntersectionObserver: trigger once per element ──
     FIX: a single shared observer instance, created once. Previously
     every MutationObserver restamp (each grid re-render, tab switch,
     batch append) built a brand-new IntersectionObserver observing ALL
     .fu-el nodes — instances piled up for the page's lifetime, each
     still watching every unrevealed card. Elements are now marked
     with data-fu-obs so each is observed exactly once. */
  var io = null;
  function initObserver() {
    if (!('IntersectionObserver' in window)) {
      /* Fallback: just show everything immediately */
      document.querySelectorAll('.fu-el').forEach(function (el) {
        el.classList.add('fu-visible');
      ...
      io = new IntersectionObserver(function (entries) {
        entries.forEach(function (entry) {
          if (entry.isIntersecting) {
            var el = entry.target;
            el.classList.add('fu-visible');
            io.unobserve(el); /* one-shot — never replays */

            /* FIX: will-change must not persist forever. Left on
               indefinitely, every revealed card stays promoted to
               its own GPU compositing layer for the rest of the
               page's life. On a gallery with many cards this stacks
               up fast and starves the raster thread during fast/fling
               scrolling, which is what causes the brief white flash
               (an unpainted layer backing store) that clears itself
               once scrolling slows. Drop the hint once the 850ms
               reveal transition is actually done, with a timeout
               fallback in case transitionend doesn't fire.
               FIX: the stagger transitionDelay is cleared here too —
               left in place it delayed every LATER transition on the
               card (hover lift, border glow) by up to 560ms. */
            var freeLayer = function () {
              el.style.willChange = 'auto';
              el.style.transitionDelay = '';
            };
            el.addEventListener('transitionend', freeLayer, { once: true });
            setTimeout(freeLayer, 900);
          }
        });
      }, {
    ...
    document.querySelectorAll('.fu-el').forEach(function (el) {
      if (el.dataset.fuObs) return; /* already being watched */
      el.dataset.fuObs = '1';
      io.observe(el);
    });
  }

  /* ── MutationObserver: catch dynamically injected cards ── */
  function watchDynamicCards() {
    if (!('MutationObserver' in window)) return;

    var mo = new MutationObserver(function (mutations) {
      var needsRestamp = false;

      mutations.forEach(function (mutation) {
        mutation.addedNodes.forEach(function (node) {
          if (node.nodeType !== 1) return;
          /* Only care about nodes inside our scoped sections */
          var inScope = SCOPE_IDS.some(function (id) {
            var sec = document.getElementById(id);
            return sec && sec.contains(node);
          });
          if (inScope) needsRestamp = true;
        });
      });
