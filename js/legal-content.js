...
<span class="lmDate">EFFECTIVE DATE: 6 JULY 2026</span>`
    ...
    shipping: {
      ...
      html: '<h2>Delivery of Digital Goods</h2>' +
        '<p>Everything sold on DigiArtz is digital. Nothing is posted, couriered or physically shipped, so no delivery address is collected and no shipping charge is ever added at checkout.</p>' +
        '<h3>When you get access</h3>' +
        '<p><strong>Immediately.</strong> Access is granted the moment the payment is confirmed, which is normally within a few seconds of paying.</p>' +
        '<ul>' +
          '<li><strong>Subscriptions</strong> — your plan is active as soon as the payment confirms, and runs for 31 days from that moment.</li>' +
          '<li><strong>Marketplace items</strong> — the download unlocks as soon as the payment confirms and stays available from My Purchases.</li>' +
        '</ul>' +
        '<h3>If access does not appear</h3>' +
        '<p>Payment confirmation can occasionally lag behind the payment itself. If your plan or download has not appeared within 30 minutes, email <a href="mailto:DigiArtzsupport@gmail.com">DigiArtzsupport@gmail.com</a> with the date and amount of the payment and we will confirm the payment and release access manually.</p>' +
        '<p>You are never charged for a delivery that does not happen: where a payment is taken but access cannot be granted, the payment is refunded in full.</p>' +
        '<span class="lmDate">LAST UPDATED: AUGUST 2026</span>'
    }
  };

if (typeof window !== 'undefined') window.DZ_LEGAL = LEGAL;
